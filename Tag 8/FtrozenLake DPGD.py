# -*- coding: utf-8 -*-
"""
Created on Wed Aug  6 15:00:51 2025
...
class Actor(nn.Module):
	def __init__(self, obs_size, hidden_size, action_size):
		...
		self.model = nn.Sequential(
			...
			nn.Softmax(dim=-1)
 # continuous action space, output in range [-1, 1]
		)
...
try:
		# pass
		actor.load_state_dict(torch.load("actorDPGD.pth", weights_only=True))
		critic.load_state_dict(torch.load("criticDPGD.pth", weights_only=True))
...
for episode in range(episodes):
	...
	if episode % 5 == 0:
		...
		# save model weights
		torch.save(actor.state_dict(), 'actorDPGD.pth')
		torch.save(critic.state_dict(), 'criticDPGD.pth')
